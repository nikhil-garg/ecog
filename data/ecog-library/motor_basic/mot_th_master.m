...
%% load data and electrodes
    ...
    load(['data/' subjects{k} '_mot_t_h'],'data','stim');
...
%% make figures
    for q={'hand','tongue'}
        for bnd={'HFB','LFB'}
            ...
            exportfig(gcf, ['figs/' subjects{k} '_GMap_' q{1} '_' bnd{1}], 'format', 'png', 'renderer', 'opengl', 'Color', 'cmyk', 'Resolution', 600, 'Width', 1.5, 'Height', 1);
            close    
        end
    end

%%    
    save(['data/' subjects{k} '_mot_th_analyzed'],'*PSD*','*FB*','tr_sc','blocksize')
    disp('-------------------------------------------')
end
%%
